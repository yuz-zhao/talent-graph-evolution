  ...
  <div class="dash" :class="{ 'anim-ready': animated }">
    <div class="hero">
      <div class="hero-left">
        ...
        <div><h1>收藏岗位</h1><p>AI 匹配历史 · 岗位收藏</p></div>
      </div>
      <div class="hero-right">
        <span class="hero-time">更新于 {{ updateTime }}</span>
        <button v-if="tab === 'matches'" class="hero-btn rematch" @click="doRematch" :disabled="rematching">
    ...
    <!-- 胶囊切换（参考管理员端系统设置） -->
    <div class="tab-bar">
      <button :class="{ active: tab === 'matches' }" @click="tab = 'matches'">匹配记录</button>
    ...
    <template v-if="tab === 'matches'">
      ...
      <div class="sc clickable tg-clickable-card" @click="tab = 'favorites'">
        ...
        <div class="timeline">
          <div class="tl-item" v-for="(m, i) in filteredMatches" :key="m.id">
            ...
            <div class="tl-dot-col">
              ...
              <div v-if="i < filteredMatches.length-1" class="tl-line"></div>
            </div>
    ...
    <template v-else>
      ...
      <template v-if="jobs.length && !loading">
        <div class="timeline">
          ...
          <div class="tl-card tg-clickable-card" @click="$router.push('/user/jobs')">
              <div class="tl-card-top">
...
.anim-ready{animation:fadeUp .3s ease both}@keyframes fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}
...
.tl-job{font-size:13px;font-weight:600;color:#1e293b;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.panel-lift{transition:all .25s cubic-bezier(.4,0,.2,1)}
